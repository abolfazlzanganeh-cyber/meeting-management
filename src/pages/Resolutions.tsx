...
import {
  ...
  AlertTriangle, Clock, CheckCircle2, Filter, Plus
} from 'lucide-react';
...
export function Resolutions() {
  ...
  if (filter === 'overdue') {
    filteredResolutions = filteredResolutions.filter(r => r.status === 'overdue' || (r.dueDate < today && r.status !== 'completed'));
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { label: string; color: string; icon: any }> = {
      pending: { label: 'در انتظار شروع', color: 'bg-slate-100 text-slate-700', icon: Clock },
      in_progress: { label: 'در حال انجام', color: 'bg-blue-100 text-blue-700', icon: TrendingUp },
      completed: { label: 'تکمیل شده', color: 'bg-green-100 text-green-700', icon: CheckCircle2 },
      overdue: { label: 'تأخیر دار', color: 'bg-red-100 text-red-700', icon: AlertTriangle },
      needs_approval: { label: 'نیازمند تأیید', color: 'bg-orange-100 text-orange-700', icon: Clock },
      cancelled: { label: 'لغو شده', color: 'bg-gray-100 text-gray-700', icon: CheckSquare },
    };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">مصوبات</h1>
          <p className="text-sm text-slate-500 mt-1">
            {filter === 'overdue' && 'مصوبات تأخیر دار'}
            {filter === 'pending-approval' && 'مصوبات نیازمند تأیید'}
            {filter === 'in-progress' && 'مصوبات در حال انجام'}
            {filter === 'completed' && 'مصوبات تکمیل شده'}
            {!filter && 'همه مصوبات'}
          </p>
        </div>
        <Button onClick={() => navigate('/meetings')} className="bg-blue-600 hover:bg-blue-700">
        ...
        <CardContent>
          {filteredResolutions.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-slate-500">مصوبه‌ای یافت نشد</p>
            </div>
          ) : (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      ...
                      <h3 className="font-semibold text-slate-900">{resolution.title}</h3>
                    </div>
                    {getStatusBadge(resolution.status)}
                  </div>

                  {resolution.description && (
                    <p className="text-sm text-slate-600 mb-3 line-clamp-2">{resolution.description}</p>
                  )}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                    ...
                    <div className="flex items-center gap-1 text-slate-600">
                      ...
                      <span>{resolution.assigneeIds.map(id => getUserName(id)).join(', ')}</span>
                    </div>
                    <div className="flex items-center gap-1 text-slate-600">
