import type { 
  User, Department, MeetingType, Meeting, Attendee, 
  Resolution, Notification, SystemSettings, AuditEntry 
} from '@/types';
...
export async function loadAllData() {
  try {
    ...
    console.log('✅ Data loaded:', cache.users.length, 'users,', cache.meetings.length, 'meetings');
  } catch (error) {
...
export const setUsers = (users: User[]) => {
  cache.users = users;
  upsertToTable('users', users);
};

// Departments
export const getDepartments = (): Department[] => cache.departments;
export const setDepartments = (deps: Department[]) => {
  cache.departments = deps;
  upsertToTable('departments', deps);
};

// Meeting Types
export const getMeetingTypes = (): MeetingType[] => cache.meetingTypes;
export const setMeetingTypes = (types: MeetingType[]) => {
  cache.meetingTypes = types;
  upsertToTable('meeting_types', types);
};
...
export const setMeetings = (meetings: Meeting[]) => {
  ...
  upsertToTable('meetings', meetings);
};

// Attendees
export const getAttendees = (): Attendee[] => cache.attendees;
export const setAttendees = (attendees: Attendee[]) => {
  cache.attendees = attendees;
  upsertToTable('attendees', attendees);
};
...
export const setResolutions = (resolutions: Resolution[]) => {
  ...
  upsertToTable('resolutions', resolutions);
};
...
export const setNotifications = (notifications: Notification[]) => {
  ...
  upsertToTable('notifications', notifications);
};

// Audit Log
export const getAuditLog = (): AuditEntry[] => cache.auditLog;
export const setAuditLog = (entries: AuditEntry[]) => {
  cache.auditLog = entries;
  upsertToTable('audit_log', entries);
};

// Settings (localStorage)
const SETTINGS_KEY = 'mms_settings';
const DEFAULT_SETTINGS: SystemSettings = {
  meetingCodePrefix: 'MTG',
  resolutionCodePrefix: 'RES',
  maxFileSize: 10,
  allowedFileTypes: ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'jpg', 'png', 'zip'],
...
export const getCurrentUser = (): User | null => {
  try {
    const data = localStorage.getItem(CU_KEY);
    return data ? JSON.parse(data) : null;
  } catch { return null; }
};
export const setCurrentUser = (u: User | null) => {
  if (u) localStorage.setItem(CU_KEY, JSON.stringify(u));
  else localStorage.removeItem(CU_KEY);
};
