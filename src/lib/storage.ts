...
export async function loadAllData() {
  try {
    const [users, departments, meetingTypes, meetings, attendees, resolutions, notifications, auditLog] = await Promise.all([
      getAllFromTable('users'),
      getAllFromTable('departments'),
      getAllFromTable('meeting_types'),
      getAllFromTable('meetings'),
      getAllFromTable('attendees'),
      getAllFromTable('resolutions'),
      getAllFromTable('notifications'),
      getAllFromTable('audit_log'),
    ]);
    
    cache.users = users || [];
    cache.departments = departments || [];
    cache.meetingTypes = meetingTypes || [];
